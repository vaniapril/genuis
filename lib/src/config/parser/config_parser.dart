...
abstract class ConfigParser {
  static GenuisConfig getConfig(File file) {
  ...
  static GenuisConfig _configFromYaml(YamlMap map) {
    ...
    final tokens = _getList<YamlMap>(map, 'tokens')?.map((e) {
          ...
          final path = _get<String>(map, 'path') ?? Defaults.tokenPathFrom(assetsPath, name);
          final type = _getType(map, 'type', ElementType.tryParse);
          if (type == null) {
            // TODO(IvanPrylepski): validate
            throw Exception('type must be specified');
          }
    ...
    final modules = _getList<YamlMap>(map, 'modules')?.map((e) {
          ...
          final path = _get<String>(map, 'path') ?? Defaults.modulePathFrom(assetsPath, name);
          final type = _getType(map, 'type', ElementType.tryParse);
          if (type == null) {
            // TODO(IvanPrylepski): validate
            throw Exception('type must be specified');
